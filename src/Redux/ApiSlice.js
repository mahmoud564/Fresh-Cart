...
export let CategoriesProduct=createAsyncThunk(
    "CategoriesHome/CategoriesProduct",
    async ()=>{
        let response=await axios.get("https://route-ecommerce.onrender.com/api/v1/categories")
        return response.data
    }
)
export let Products=createAsyncThunk(
...
export let ProductsDetails=createAsyncThunk(
    ...
    async (param)=>{
        let response=await axios.get(`https://route-ecommerce.onrender.com/api/v1/Products/${param}`)
        return response.data
    }
)
let initialState={
